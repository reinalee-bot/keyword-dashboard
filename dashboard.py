...
BUILD: 2026-06-30-KEYWORD-V4

V4 수정 내역:
- _arr 깨진 텍스트: font-family span 범위 축소 + data-testid / class*=_arr 3중 차단
- 활용처 관리: 행별 저장버튼 제거 → on_change 자동저장
- 필터: 버튼형 세그먼트 (파란=선택)
- 키워드 칩: 칩 본체 + X 통합 (칩 3열 그리드)
- 트렌드 카드: 독립 통계 + 통일 높이 + 분석 대기 메시지
- 기사 탭: 버튼 레이아웃 정리 / 마지막 업데이트 표시
- 빠른 등록: form 내 expander 정상화, 자동 추적 옵션 추가
"""
...
# ── 상수 ──────────────────────────────────────────────────
BUILD_VERSION = "2026-06-30-KEYWORD-V4"
...
/* ─ 폰트: 구체적 HTML 태그만 지정 (span 전역 지정 금지 → _arr 텍스트 노출 방지) ─ */
...
/* ══ expander 화살표 텍스트 3중 차단 ══

   원인: Streamlit 내부 span 에 CSS 모듈 클래스(_arr... 등)가 붙고,
         span 전역 font-family 적용 시 해당 클래스명이 텍스트로 노출됨.
   대책 1 — data-testid 로 화살표 span 직접 지정                      */
...
/* 대책 2 — class 이름에 _arr 포함되는 모든 요소 */
[class*="_arr"] {
  font-size: 0 !important;
  color: transparent !important;
  line-height: 0 !important;
...
/* 대책 3 — summary 직계/후손 span (화살표 컨테이너) */
...
/* ─ expander 외관 ─ */
[data-testid="stExpander"] > details { border: 1px solid #DCE3EA; border-radius: 8px; overflow: hidden; margin: .4rem 0; }
...
.tc-wait { padding:24px 0; text-align:center; color:#94A3B8; font-size:13px; }
.tc-wait-title { font-weight:700; color:#667085; margin-bottom:4px; }
...
def _status_html(s):
    if s == "반영완료":
        return "<span class='tag-done'>반영완료</span>"
    return "<span class='tag-todo'>도출</span>"
...
def content_dialog(keyword, month, usage):
    ...
    if st.button("활용처 저장", key=f"dlg_us_{keyword}_{month}", type="secondary"):
        if update_usage_type(keyword, month, nu):
            st.success(f"'{nu}'로 변경했습니다."); _inv_derived(); st.rerun()
        else: st.error("저장 실패.")
    st.markdown("---")
...
def _make_usage_cb(kw, month):
    """on_change 콜백 — 클로저로 키워드/월 캡처. 별도 저장 버튼 불필요."""
    def _cb():
...
<div class="s">도출 목표 5건 · 반영률 목표 70%</div></div>""", unsafe_allow_html=True)
    ...
    with c4:
        ...
        hint= ("두 목표 모두 달성" if KPI_OK
               else f"도출 {max(KPI_TD-KPI_D,0)}건 · 반영률 {max(KPI_TR-RATE,0)}%p 부족")
        st.markdown(f"<div class='kpi-card'><div class='kpi-lbl'>KPI 달성</div>"
                    f"<span class='{bc}'>{bt}</span>"
                    f"<div class='kpi-hint' style='margin-top:10px'>{hint}</div></div>",
                    unsafe_allow_html=True)
    ...
    with st.expander("월별 KPI 누적 현황", expanded=False):
        ...
        with pd.ExcelWriter(buf_m, engine="openpyxl") as w:
            dfm.to_excel(w, index=False, sheet_name="월별KPI")
        cc, _ = st.columns([2,5])
        with cc:
...
<div class="s">이번 달 발굴한 키워드를 즉시 등록합니다.</div></div>""", unsafe_allow_html=True)
    ...
    with st.form("t2_qreg", clear_on_submit=True):
        ...
        with qa:
            q_kw = st.text_input("키워드 *", placeholder="예: 제로트러스트", label_visibility="visible")
        with qb:
            q_us = st.selectbox("활용처 *", USAGES, label_visibility="visible")
        with qc:
            st.markdown("<div style='height:28px'></div>", unsafe_allow_html=True)
            q_sub = st.form_submit_button("＋ 키워드 등록", use_container_width=True, type="primary")

        # expander inside form — Streamlit 1.40+ 지원
        with st.expander("추가 정보 입력 (선택)"):
            e1, e2 = st.columns(2)
            with e1:
                q_ve = st.text_input("관련 벤더", placeholder="예: Palo Alto Networks")
                q_id = st.text_input("아이디어·메모", placeholder="예: 이달 보도자료 핵심 소재")
            with e2:
                q_su = st.text_input("출처 URL", placeholder="https://...")
                q_at = st.checkbox("등록 후 자동 추적 시작", value=True,
                                   help="체크 시 트렌드 탐색 탭에도 즉시 추가됩니다.")

    # form 바깥에서 처리 (form submit 이후 변수값은 유효)
    if q_sub:
        kw_t = (q_kw or "").strip()
        if not kw_t:
            st.warning("키워드를 입력해 주세요.")
        elif add_keyword(kw_t, CURRENT_MONTH, usage_type=q_us,
                         vendor=(q_ve or "").strip(),
                         idea=(q_id or "").strip(),
                         source_url=(q_su or "").strip()):
            _inv_derived()
            if q_at:
...
<div class="s">키워드 클릭 → 그래프 포함/제외 &nbsp;·&nbsp; × 클릭 → 추적 삭제 &nbsp;·&nbsp; 아래에서 새 키워드 추가</div></div>""",
                unsafe_allow_html=True)
    ...
    if tracked_kws:
        # ── 상단 액션 버튼 ─────────────────────────────
        ba_, bb_, bc_, bd_ = st.columns([1.8,1.8,2.2,4])
        with ba_:
        ...
        for row_start in range(0, len(tracked_kws), CHIP_COLS):
            ...
            for j, kw in enumerate(batch):
                ...
                with chip_cols[j * 2]:
                    if st.button(
                        f"{'●' if not hid else '○'} {kw}",
                        key=f"chip_body_{kw}",
                        type="primary" if not hid else "secondary",
                        use_container_width=True
                    ):
                        if hid: hidden_kws.discard(kw)
                        else:   hidden_kws.add(kw)
                        st.session_state["hidden_kws"] = hidden_kws
                        st.rerun()
                with chip_cols[j * 2 + 1]:
                    if st.button("×", key=f"chip_x_{kw}",
                                 type="secondary", use_container_width=True,
                                 help=f"'{kw}' 추적 삭제"):
                        remove_tracked_keyword(kw)
                        hidden_kws.discard(kw)
                        st.session_state["hidden_kws"] = hidden_kws
                        _inv_tracked(); st.rerun()
...
<div class="s">최대 5개 키워드 동시 비교</div></div>""", unsafe_allow_html=True)
    ...
    if not sel_kws:
        ...
        src_choice = st.radio("소스", ["네이버 데이터랩","구글 트렌드"],
        ...
        if vis_kws:
            ...
            if not dw.empty:
                ...
                fig = px.line(dw, x="주차", y="평균 관심도", color="키워드",
                              markers=True, line_shape="spline", height=380,
        ...
        # ══ 키워드별 추이 요약 카드 ══
        # STEP 1: 모든 키워드 통계 사전 계산 (렌더링 전 완전 격리)
...
<div class="s">최근 {period_days}일 · 각 키워드 독립 처리</div></div>""", unsafe_allow_html=True)
        ...
        for _kw in sel_kws:
            ...
            _pre[_kw] = {
                "n":  compute_kw_stats(_dn),
                "g":  compute_kw_stats(_dg),
                "dn": _dn,
                "dg": _dg,
            }

        # STEP 2: 사전 계산 결과만 사용해 렌더링
...
트렌드 데이터가 확보되면 자동으로 분석됩니다.</div></div>""", unsafe_allow_html=True)
                        continue
                    ...
                    try:
                        peak_str = pd.Timestamp(_ref_df.loc[_ref_df["ratio"].idxmax(),"date"]).strftime("%Y.%m.%d")
                    except: peak_str = "—"
                    try:
                        last_str = pd.Timestamp(_ref_df["date"].max()).strftime("%Y.%m.%d")
                    except: last_str = "—"
...
""", unsafe_allow_html=True)
                    ...
                    if _sp is not None:
                        ...
                        st.plotly_chart(_sparkline(_sp, _col_c),
                                        use_container_width=True,
                                        config={"displayModeBar": False},
                                        key=_sp_key)

        with st.expander("원본 트렌드 데이터 보기", expanded=False):
            df_raw = df_period[df_period["source"] == src_key].copy() if "df_period" in dir() else pd.DataFrame()
            if not df_raw.empty:
...
<div class="s">네이버 뉴스 검색 API · 주요 제휴·우선 매체 화이트리스트 · 화제성 추정 점수 제공</div></div>""",
    ...
    with st.container(border=True):
        ...
        with fc:
            t4_scope = st.selectbox("언론사 범위",
                                    ["주요 제휴·우선 매체만","전체 뉴스 검색 결과"],
                                    key="t4_scope")
        with fd:
            t4_type  = st.selectbox("기사 유형",
                                    ["전체","보도자료형","기획·분석","인터뷰","행사·현장","일반 기사"],
                                    key="t4_type")
        with fe:
        ...
        with btn_a:
            t4_fetch = st.button("이번 주 기사 불러오기", type="primary",
                                  use_container_width=True, key="t4_fetch_btn",
                                  disabled=not t4_sel_kws)
        with btn_b:
            t4_reset = st.button("결과 지우기", type="secondary",
                                  use_container_width=True, key="t4_rf_btn")
    ...
    if t4_fetch and t4_sel_kws:
        ...
        new_results: dict = {}; new_clusters: dict = {}
        ...
        for i, kw in enumerate(t4_sel_kws):
            ...
            _res = nf.fetch_articles_for_keyword(
                ...
                sort_api=sort_api, media_scope=scope_key, article_type_filter=type_f,
...
조회 조건을 설정하고 '이번 주 기사 불러오기'를 클릭하면 기사를 불러옵니다.
        ...
        for kw, clusters in t4_clusters.items():
            ...
            _err    = _res.get("error","")
            cnt_str = (f"{_res.get('filtered_count',0)}건 선별"
                       if _status == "success"
...
<div class="s">활용처 변경 시 자동 저장 · 상세 편집 버튼으로 콘텐츠 등록·수정</div></div>""",
        ...
        for _, row in df5.iterrows():
            ...
            with r1:
                ...
                st.selectbox(
                    ...
                    on_change=_make_usage_cb(kw, CURRENT_MONTH),
                )
                # 저장됨 표시 (1회성 플래그)
                if st.session_state.pop(f"t5_saved_{kw}", False):
                    st.markdown("<span style='color:#059669;font-size:11px'>✓ 저장됨</span>",
                                unsafe_allow_html=True)
            with r2:
                st.markdown(_status_html(stat), unsafe_allow_html=True)
            with r3:
                if cn:
                    disp = cn + (f" 외 {ctot-1}건" if ctot > 1 else "")
                    st.markdown(f"<span class='td'>{disp}</span>", unsafe_allow_html=True)
                else:
                    st.markdown("<span class='td' style='color:#94A3B8'>—</span>", unsafe_allow_html=True)
            with r4:
                if cu:
                    st.markdown(f"[↗]({cu})", unsafe_allow_html=True)
                else:
    ...
    with st.expander("키워드 삭제 (주의)", expanded=False):
        st.caption("잘못 등록된 키워드를 삭제합니다. 해당 키워드의 콘텐츠 기록도 함께 삭제됩니다.")
    ...
    with st.expander("월별 KPI 누적 현황", expanded=False):
        ...
        with pd.ExcelWriter(buf5, engine="openpyxl") as w:
            dfm5.to_excel(w, index=False, sheet_name="월별KPI")
